...
try {
  ...
  const salt=randomBytes(16), iterations=310000;
  ...
  writeFileSync(sqlFile,`INSERT INTO admin_users (id,email,display_name,password_hash,active,created_at,updated_at) VALUES (${quote(randomUUID())},${quote(email)},${displayName?quote(displayName):"NULL"},${quote(passwordHash)},1,${quote(now)},${quote(now)});\n`,{mode:0o600});
