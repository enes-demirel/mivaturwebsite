  ...
  return (
    <div className="space-y-14 sm:space-y-16">
      ...
      <DetailSection id="visited-places" title="Gezilecek ülkeler ve şehirler">
        <div className="grid gap-5 sm:grid-cols-2"><PlaceList title="Ülkeler" items={detail.visitedCountries} /><PlaceList title="Şehirler" items={detail.visitedCities} /></div>
      </DetailSection>
      ...
      {(detail.hotelInformation || detail.mealInformation) && (
        <DetailSection id="stay-and-meals" title="Konaklama ve yemek">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            {detail.mealInformation && <div className="rounded-lg border border-border bg-surface p-5 sm:p-6"><h3 className="text-lg font-bold text-text">Yemek bilgisi</h3><ul className="mt-4 space-y-3">{detail.mealInformation.map((item) => <li key={item} className="flex gap-2 text-sm leading-6 text-muted"><span aria-hidden="true" className="text-brand">•</span>{item}</li>)}</ul></div>}
          </div>
        </DetailSection>
      )}
    </div>
...
export function TourDetailSections({ detail, similarTours }: { detail: TourDetail; similarTours: readonly Tour[] }) {
  return (
    <div className="mt-14 space-y-14 sm:mt-16 sm:space-y-16 lg:space-y-20">
      ...
      {detail.pdfUrl && <DetailSection id="tour-pdf" title="Tur programı PDF’i"><div className="flex flex-wrap gap-3"><a href={detail.pdfUrl} download className="inline-flex min-h-11 items-center rounded-md bg-brand px-5 text-sm font-bold text-white">Tur Programını PDF Olarak İndir</a><Link href={detail.pdfUrl} target="_blank" className="inline-flex min-h-11 items-center rounded-md border border-border px-5 text-sm font-bold">Yeni sekmede görüntüle</Link></div></DetailSection>}
