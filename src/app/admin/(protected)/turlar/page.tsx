...
export default async function AdminToursPage({ searchParams }: ToursPageProps) {
  ...
  const { data, error } = await supabase.from("tours").select("id, title, slug, type, status, duration_days, duration_nights, updated_at, tour_departures(start_date, price, currency, status)");
  const allTours = data ?? [];
